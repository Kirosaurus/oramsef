...
// === FUNGSI OVERRIDE SETTIMEOUT UNTUK BACKGROUND ===
// Memanfaatkan Web Worker agar setInterval tidak di-throttle oleh browser
const hiddenWorker = new Worker(URL.createObjectURL(new Blob([`
    let intervalId;
...
function startUnthrottledInterval(callback, ms) {
    hiddenWorker.onmessage = callback;
    hiddenWorker.postMessage({ command: 'start', ms: ms });
}

function stopUnthrottledInterval() {
    hiddenWorker.postMessage({ command: 'stop' });
}
// ====================================================
...
function startCameraAndWebsocket() {
    ...
    // Pastikan koneksi yang lama tertutup bersih jika pengguna klik sangat cepat
    if (ws) {
        ws.close();
        ws = null;
    }
    ...
    ws.onmessage = (event) => {
        ...
        // Tampilkan frame video dari backend ke element image di frontend
        if (data.frame_base64) {
            videoElement.src = data.frame_base64;
        }
...
// Menggunakan WebSocket persisten agar tahan terhadap minimize browser.
let kaWs = null;
let keepAliveReconnectTimer = null;

function connectKeepAlive() {
    if (kaWs && (kaWs.readyState === WebSocket.OPEN || kaWs.readyState === WebSocket.CONNECTING)) {
        return;
    }

    kaWs = new WebSocket("ws://localhost:8000/ws/keepalive");
    ...
    kaWs.onerror = () => {
        // Abaikan error di console
    };
}
connectKeepAlive();

// Penting: Cegah browser memutus WebSocket saat tab di-background/sleep
document.addEventListener("visibilitychange", () => {
    if (document.visibilityState === 'visible') {
        connectKeepAlive(); // Pastikan koneksi terjaga saat tab kembali difokuskan
    }
});
